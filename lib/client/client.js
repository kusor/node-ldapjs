...
function setupSocket(socket, opts, emitter) {
    var log = opts.log;
    ...
    if (typeof (socket.setKeepAlive) !== 'function') {
        socket.setKeepAlive = function setKeepAlive(enable, delay) {
            return socket.socket ? socket.socket.setKeepAlive(enable, delay) :
                false;
        };
    }
    ...
    socket.once('close', function onClose(had_err) {
...
function createSocket(opts, emitter, cb) {
    var log = opts.log;
    ...
    function _setupSocket(_, _cb) {
        function onSocketConnect() {
            ...
            socket.removeListener('secureConnect', onSocketConnect);
            assert.ok(socket.ldap);

            socket.ldap.id = nextClientId() + '__' + socket.ldap.id;
            log = log.child({ldap_id: socket.ldap.id}, true);

            log.trace('connect event');

            // emitter.emit('connect', socket);
            socket.removeListener('error', onSocketError);
            emitter.removeListener('connectTimeout', emitterConnectTimeout);
        ...
        function emitterConnectTimeout(err) {
            if (!socket || !socket.readable || !socket.writeable) {
                socket.destroy();
            }
            onSocketError(new Error('connect timeout'));
        }

        var proto = opts.secure ? tls : net;
        socket = proto.connect(
                (opts.port || opts.socketPath),
                opts.host,
                opts.secure ? opts.tlsOptions : null);
        setupSocket(socket, opts, emitter);
        socket.once('connect', onSocketConnect);
        socket.once('secureConnect', onSocketConnect);
        socket.once('error', onSocketError);
        emitter.once('connectTimeout', emitterConnectTimeout);
    }

    var retry = backoff.call(_setupSocket, null, cb);
    ...
    retry.on('backoff', function (number, delay) {
        ...
        log[level]({
            attempt: number,
            delay: delay
        }, 'ldapjs client: connection attempt failed');
    });
    ...
    if (opts.connectTimeout) {
        timer = setTimeout(function onConnectTimeout() {
            retry.abort();
            emitter.emit('connectTimeout', new Error('client connectTimeout'));
        }, opts.connectTimeout);
    }
...
function Client(options) {
    ...
    // Need to remove log from options before calling clone on it or
    // and exception regarding log socket will be raised.
    delete options.log;
    // Clone the options for debugging purposes
    this.ctor_options = clone(options);
    ...
    // Bring our logger back to options:
    options.log = self.log;

    options.connectTimeout = parseInt((options.connectTimeout || 0), 10);
    options.host = _url ? _url.hostname : undefined;
    options.port = _url ? _url.port : false;
    options.secure = _url ? _url.secure : false;
    options.socketPath = options.socketPath || false;
    options.timeout = parseInt((options.timeout || 0), 10);

    (function connect() {
        var d = domain.create();
        d.on('error', function (err) {
            options.log({err: err}, 'ldapjs client constructor domain error');
            self.emit('error', err);
        });

        d.run(function () {
            self.connecting = createSocket(options, self,
                function (err, socket) {
                self.connecting = false;

                // We only get error if credentials are invalid or there is no
                // server listening on the provided socket/url
                if (err) {
                    self.emit('error', err);
                    return;
                }

                if (self.closed && socket) {
                    return;
                }

                function handleClose() {
                    if (self.socket && !self.connecting && !self.closed) {
                        self.log.warn(err, 'LDAP client disconnected');
                        self.socket = null;
                        connect();
                    }
                }
                // Wouldn't be 'close' called right after this event?
...
/**
 * Closes the underlying socket and stops retrying connections.
 *
 * @param {Function} callback of the form f(err).
 */
Client.prototype.close = function close(cb) {
    ...
    if (options.attributes) {
        if (!Array.isArray(options.attributes)) {
            if (typeof (options.attributes) === 'string') {
                ...
                throw new TypeError(
                        'options.attributes must be an Array of Strings');
            }
        }
    }
...
Client.prototype._send = function _send(message, expect, emitter, callback) {
        ...
        if (Array.isArray(msg.controls) && msg.controls.length > 0) {
            ...
            for (var i = 0; i < msg.controls.length; i++) {
                ...
                if (resControl instanceof PagedResultsControl) {
                    ...
                    if (resControl.value.cookie &&
                            resControl.value.cookie.length > 0) {
                        ...
                        if (Array.isArray(message.controls) &&
                            ...
                            for (var j = 0; j < message.controls.length; j++) {
                                ...
                                if (reqControl instanceof
                                        PagedResultsControl) {
                                    // update request cookie and re-send
                                    reqControl.value.cookie =
                                        resControl.value.cookie;
    ...
    reqd.on('error', function (err) {
        log({err: err}, 'ldapjs client req domain error');
        self.emit('error', err);
    });
